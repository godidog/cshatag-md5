...
func storeAttr(bMd5 bool, f *os.File, attr fileAttr) (err error) {
	if args.dryrun {
		return nil
	}
	...
	if bMd5 {
		err = xattr.FSet(f, xattrTsMd5, []byte(attr.tsMd5.prettyPrint()))
		if err != nil {
			return
		}
		err = xattr.FSet(f, xattrMd5, attr.md5)
	} else {
		err = xattr.FSet(f, xattrTsSha256, []byte(attr.ts.prettyPrint()))
		if err != nil {
			return
		}
...
func removeAttr(f *os.File) error {
	...
	err1 := xattr.FRemove(f, xattrTsSha256)
